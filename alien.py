...
class Alien(pygame.sprite.Sprite):    #pygame.sprite.Sprite: des fonctionnalités prédéfinies pour les sprites dans Pygame
    def _init_(self,color,x,y): #Sélection des paramètres
        ...
        file_path = '../graphics/' + color + '.png' #Crée un chemin vers l'image de l'alien 
        self.image = pygame.image.load(file_path).convert_alpha() #pygame.image.load(file_path) = Charge l'image depuis le chemin spécifié - .convert_alpha() : Gère la transparence de l'image, pour que les parties transparentes soient bien rendues.
